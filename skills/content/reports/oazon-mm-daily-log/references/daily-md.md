...
- **MOCREO V2**
    - ...
                                  ...
                                  ← 若干空行（实测 2–4 个，别当成固定值）
---                               ← 之后是与日报无关的草稿区
（卡片 id 清单、待办 checkbox、零散笔记……）
```

**新块的落点是「最后一个日期块的最后一条子条目之后」**，不是文件末尾。文末那一大段是用户自己的草稿区，别碰。

⚠️ **别拿 `---` 当锚点**。全文件有 70+ 条 `^---`（2025 年的日期块之间也用它分隔，草稿区里还有几条，个别行尾带空格所以 `^---$` 匹配不到）。可靠的定位方式只有一条：

```bash
grep -nE '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$' "$T" | tail -1   # 最后一个日期头的行号 D
awk -v d=D 'NR>d && /^    - /{n=NR} END{print n}' "$T"     # 它之后最后一条子条目的行号 N
```

新块接在第 N 行之后即可，N 以下的空行、`---`、草稿区原样保留。

老的 2025 年条目是编号列表且没有工时。**不要拿它当格式依据**，照最近几天的写。

## 格式约定

实测自 2026.08.17–08.19 三个日期块：

| 项 | 约定 | 注意 |
|---|---|---|
| 日期头 | 裸行 `2026.08.20`，无 bullet、无 `##` | **年.月.日，月/日补前导零**。不是 `8.20`（2026-08-31 已把全文件 324 个旧日期头统一改过来），也不是 `2026-08-20` |
| 项目行 | `- **项目名**` | 不带工时 |
| 子条目缩进 | **4 个空格** | 不是 tab。历史上有个别行混了 tab，别跟着学 |
| 工时括号 | **全角** `（0.5h）` | 有少数历史行用了半角，以全角为准 |
| 日期块之间 | 一个空行 | 末尾那块与 `---` 之间是 2–4 个空行，不固定；插新块时别去动它们 |
...
# 2. 找落点：最后一个日期块的最后一条子条目行号 N
#    只认日期头 + 子条目，不要用 `---` 定位（全文件 70+ 条，会认错）
D=$(grep -nE '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$' "$T" | tail -1 | cut -d: -f1)
N=$(awk -v d="$D" 'NR>d && /^    - /{n=NR} END{print n}' "$T")
echo "最后日期头 $D，最后子条目 $N"; sed -n "${D},${N}p" "$T" | tail -5   # 眼核一遍

# 3. 在 scratchpad 里拼新文件（$DATE = 报告日期，如 2026-09-01；$D 是行号，别混用）
head -n "$N" "$SP/daily.orig.md" > "$SP/daily.new.md"
cat "$SP/block.md" >> "$SP/daily.new.md"     # block.md 以一个空行开头
tail -n +$((N+1)) "$SP/daily.orig.md" >> "$SP/daily.new.md"
sed -i "s/^updated: .*/updated: $DATE/" "$SP/daily.new.md"
